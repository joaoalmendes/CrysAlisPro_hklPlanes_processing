...
def detect_peaks(data, min_distance=50, threshold_rel=0.50):
...
temp = "15K"  # Hardcoded for testing
if temp == "80K":
    peak_dir = "6peaks"
elif temp == "15K":
    peak_dir = "7peaks"
        ...
        for peak in peaks:
            try:
                ...
                for existing_hkl in list(peak_dict.keys()):
                    if all(abs(hkl_rounded[i] - existing_hkl[i]) <= 0.2 for i in range(3)):
                        # Same peak; update if intensity is higher
                        if intensity > peak_dict[existing_hkl]['intensity']:
                            del peak_dict[existing_hkl]
                            peak_dict[hkl_tuple] = {
                                'intensity': intensity,
                                'j': peak['j'],
                                'i': peak['i'],
...
if peak_data:
    df = pd.DataFrame(peak_data, columns=['j/x', 'i/y', 'data frame', 'h', 'k', 'l'])
